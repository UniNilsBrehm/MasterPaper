...
\notenb{SuppFig 1 C to F}
\notenb{Mention first EPhys and an overview of the auditory system, Fig2A,B}
First we characterized the auditory system by establishing intensity and frequency tuning curves based on the neuronal activity in response to sine waves of different intensity or frequency in the two species \species{Carales astur} and \species{Estigmene acrea}. With increasing intensity at a fixed frequency the firing rate of the auditory receptor cells increased in a sigmoidal fashion that can be well described by a Boltzmann function. By extrapolating the tangent at $I_{0}$, a threshold can be defined by the intersect of that tangent and the minimum ($f_{min}$) value. The dynamic range the auditory neuron covered spanned around 20~dBSPL (linear part of the tuning curve), encoded by firing rates that reached up to 400~Hz \suppfig{\ref{fig:01}}{C}. By computing such tuning curves for a broad range of carrier frequencies we determined the overall frequency tuning of the auditory neurons. Both \species{Carals astur} and \species{Estigmene acrea} showed a broad range, reaching from 10~kHz to 100~kHz. However, \species{Carales astur} was tuned more towards the high frequencies, whereas \species{Estigmene acrea} has a more equal distribution over the same range \suppfig{\ref{fig:01}}{E, F}.

Since moths are not only exposed to signals coming from their con-specifics, we compared the frequency tuning of the auditory system with the power spectra of not only their own calls but also several bat species. Interestingly, the power spectrum of several bats averaged is distributed very broadly, matching the range of the moth's frequency tuning, showing it is a matched filter. The power spectra from calls of the two moth species used in this study were even broader, covering up to 100~kHz.  \suppfig{\ref{fig:01}}{D}.
  
\subsection{Temporal resolution}
\notenb{This is Fig 2 C to H and SuppFig 2}
...
\notenb{This is Fig 3 C and D and SuppFig 3}
