...
\notenb{This is Fig 1 and SuppFig 1 A and B}
...
A single pulse can be described by a damped sine wave with a carrier frequency and duration that can vary between different species. In all calls used in this study the pulse duration ranged from 0.1~ms to 0.6~ms \fig{\ref{fig:01}}{C}, therefore all of them were shorter than a millisecond, a duration that is in the same scale of action potentials. The carrier frequency was broadly distributed over different species, ranging from 5~kHz up to 80~kHz  \suppfig{\ref{suppfig:01}}{A,B}.
As the moth tymbal contracts it produces active pulse train and as it relaxes again a passive pulse train \fig{\ref{fig:01}}{A} \notenb{Maybe add labels in Fig 1 A for active and passive} defining one call. The inter pulse interval (IPI), i.e the time between pulses, was mostly below 5~ms \fig{\ref{fig:01}}{D}and the inter train interval (ITI) varied from 2~ms up to 80~ms \fig{\ref{fig:01}}{F}.

In nature, calls rarely occur isolated, but are repeated in call series \fig{\ref{fig:01}}{B}. Therefore, they are the natural signals of arctiid moths. The duration of call series (audio recordings) used in this study ranged from 0.14~s to 3.99~s \fig{\ref{fig:01}}{G} containing mostly around 100 to 200 pulses \fig{\ref{fig:01}}{E}.

However, calls within a call series are not distributed uniformly. In many species, calls occur in "packages", where the intervals between calls within a package are short, while the intervals between packages are much longer. As a result, the acoustic signals of arctiid moths have a modular structure. They are organized hierarchically, consisting of single pulses (inter-pulse intervals), pulse trains (inter-pulsetrain intervals), individual calls, packages of calls, and ultimately the entire call series. This generates multiple temporal scales that receptor neurons could be tuned to, potentially allowing discrimination between calls of different species.
...
First we characterized the auditory system by establishing intensity and frequency tuning curves based on the neuronal activity in response to sine waves of different intensity or frequency in the two species \species{Carales astur} and \species{Estigmene acrea}. With increasing intensity at a fixed frequency the firing rate of the auditory receptor cells increased in a sigmoidal fashion that can be well described by a Boltzmann function. By extrapolating the tangent at $I_{0}$, a threshold can be defined by the intersect of that tangent and the minimum ($f_{min}$) value. The dynamic range the auditory neuron covered spanned around 20~dBSPL (linear part of the tuning curve), encoded by firing rates that reached up to 400~Hz \suppfig{\ref{suppfig:01}}{C}. By computing such tuning curves for a broad range of carrier frequencies we determined the overall frequency tuning of the auditory neurons. Both \species{Carals astur} and \species{Estigmene acrea} showed a broad range, reaching from 10~kHz to 100~kHz. However, \species{Carales astur} was tuned more towards the high frequencies, whereas \species{Estigmene acrea} has a more equal distribution over the same range \suppfig{\ref{suppfig:01}}{E, F}.

Since moths are not only exposed to signals coming from their con-specifics, we compared the frequency tuning of the auditory system with the power spectra of not only their own calls but also several bat species. Interestingly, the power spectrum of several bats averaged is distributed very broadly, matching the range of the moth's frequency tuning, showing it is a matched filter. The power spectra from calls of the two moth species used in this study were even broader, covering up to 100~kHz.  \suppfig{\ref{suppfig:01}}{D}.
  
\subsection{Temporal resolution}
% This is Fig 2 C to H and SuppFig 2
The temporal structure of moth calls is the major parameter for a discrimination between different species. Therefore we recorded the response of auditory neurons of \species{Estigmene acrea} to rectangular stimuli and model pulses with different gap length \suppfig{\ref{suppfig:02}}{A}.
...
When we now decreased the gap size, the spiking pattern became more irregular and less locked to the stimulus period indicated by lower mean vector strength values. Figure~2~D shows the mean vector curve for three different individuals. A gap detection threshold was determined by the smallest gap size that still yielded a significant mean vector strength. For rectangular pulse trains with a pulse duration of 10~ms this threshold was around 2~ms to 3~ms \fig{\ref{fig:02}}{D}, where as for s pulse duration of only 5~ms it increased to around 5~ms \fig{\ref{fig:02}}{E}.

Using artificial moth pulses \fig{\ref{fig:02}}{F} with a duration of 2~ms and 0.5~ms produced gap detection thresholds of 5~ms \fig{\ref{fig:02}}{G} and 7~ms \fig{\ref{fig:02}}{H}, respectively. In contrast to the rectangular pulses with their longer pulse duration, the spikes were now locked to the stimulus with a phase shift of 180~deg \suppfig{\ref{suppfig:02}}{F} and the locking was one spike per period \suppfig{\ref{suppfig:02}}{E, G}. 

When exposing the animal to a constant pure tone with no gaps in between, the temporal pattern vanished completely and the response resembles a typical adaptation to constant sounds \suppfig{\ref{suppfig:02b}}{}.
...
\end{figure}
\clearpage
\captionof{figure}{ Temporal resolution of auditory neurons A1/A2.
...
% Zhis is Fig 3 C and D and SuppFig 3
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The 3 Main Figures
\notenb{Match figure size to page (including caption)}
% Supp Figures
\begin{suppfigure}
	\centering
